#ifndef SANDBOX_GRAPHICS_VULKAN_IMAGE_VULKAN_IMPORT_IMAGE_H_
#define SANDBOX_GRAPHICS_VULKAN_IMAGE_VULKAN_IMPORT_IMAGE_H_
...
class VulkanImportImage : public VulkanImage {
public:
	VulkanImportImage(int externalHandle) : image(NULL), external(true), externalHandle(externalHandle) { addType<VulkanImportImage>(); }
	virtual ~VulkanImportImage() {}
...
protected:
	void startRender(const GraphicsContext& context, VulkanDeviceState& state) {
		...
		if (state.getRenderMode().get() == VULKAN_RENDER_UPDATE_SHARED) {
			...
			createImage(state.getDevice(), texWidth, texHeight, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, imageState->image, imageState->imageMemory, external, externalHandle);
    ...
    static void createImage(const VulkanDevice* device, uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, bool external, int externalHandle) {
        ...
        VkImportMemoryFdInfoKHR importMemAlloc;
        importMemAlloc.sType = VK_STRUCTURE_TYPE_IMPORT_MEMORY_FD_INFO_KHR;
        importMemAlloc.pNext = NULL;
        importMemAlloc.handleType = VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT;
        importMemAlloc.fd = externalHandle;

        VkMemoryAllocateInfo allocInfo = {};
        allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        if (external) {
            allocInfo.pNext = &importMemAlloc;
        }
